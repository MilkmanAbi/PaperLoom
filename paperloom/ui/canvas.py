    ...
    def eventFilter(self, watched, event):
        ...
        if et == QEvent.Type.MouseButtonPress:
            ...
            if self._drag_offset is not None and (event.buttons() & Qt.MouseButton.LeftButton):
                # dragging happens in the live widget's own (zoomed) pixel
                # space - it's what's actually on screen - but the model
                # (and the snap grid, and alignment guides against other
                # widgets) is always in unscaled logical units, so a drag
                # under zoom converts pixel -> logical -> snapped logical,
                # then converts back to pixel only for the on-screen move().
                delta = event.position().toPoint() - self._drag_offset
                target_px = watched.pos() + delta
                nx = self.canvas.snap(self.canvas.to_logical(target_px.x()))
                ny = self.canvas.snap(self.canvas.to_logical(target_px.y()))
                ...
                if dw is not None:
                    ...
                    self.canvas.set_guides(
                        self.canvas.to_px(gx) if gx is not None else None,
                        self.canvas.to_px(gy) if gy is not None else None)
                watched.move(self.canvas.to_px(nx), self.canvas.to_px(ny))
...
class DesignCanvas(QWidget):
    ...
    def __init__(self, registry, page: DesignPage = None, parent=None, undo_stack=None):
        ...
        self.zoom_factor = 1.0     # 1.0 = 100%; see to_px()/to_logical()
    ...
    def set_undo_stack(self, undo_stack):
        """Swap which UndoStack model-mutating commands are pushed onto.
        Tabs (spec: Tabs, Phase A) give each open page its own independent
        undo/redo history - the window creates one UndoStack per page and
    ...
    # --- zoom -------------------------------------------------------------
    # The model (DesignWidget.x/y/width/height, and everything codegen reads)
    # is always in unscaled LOGICAL units - what the generated app's real
    # pixel geometry will be, regardless of what zoom level you happened to
    # be editing at. Live widgets on screen are positioned in PIXEL units -
    # logical * zoom_factor. to_px()/to_logical() are the one conversion
    # boundary every interaction (drag, resize, align, snap) routes through,
    # so "zoom" actually rescales the live widgets and the grid, rather than
    # just changing a label that nothing reads (the previous behaviour).
    def to_px(self, logical):
        return round(logical * self.zoom_factor)

    def to_logical(self, px):
        return round(px / self.zoom_factor) if self.zoom_factor else int(px)

    def snap_px(self, px):
        """Snap a PIXEL coordinate to the logical grid, then convert back -
        so the resize handles (which drag in live pixel space) snap to the
        same grid the canvas draws and the move-drag path snaps to, at any
        zoom level."""
        return self.to_px(self.snap(self.to_logical(px)))

    def _apply_live_geometry(self, dw, live):
        """The one place a live widget's on-screen geometry is set from the
        model - always dw's logical units scaled by the current zoom."""
        live.setGeometry(self.to_px(dw.x), self.to_px(dw.y),
                          self.to_px(dw.width), self.to_px(dw.height))

    def set_zoom(self, percent):
        """Rescale every live widget (and the grid) to `percent`% - called by
        the top bar's zoom dropdown / Ctrl+scroll / View > Zoom In-Out-Reset.
        Model coordinates never change; only how they're rendered does."""
        self.zoom_factor = max(0.1, percent / 100.0)
        for dw in list(self.page.widgets):
            live = self._live_by_model.get(id(dw))
            if live is not None:
                self._apply_live_geometry(dw, live)
        self.sync_overlay()
        self.update()
    ...
    def load_page(self, page):
        ...
        # tear down current live widgets - hide + reparent immediately rather
        # than relying on deleteLater() alone: deferred deletion only actually
        # runs once the event loop gets back around to it, so a widget from
        # the OLD page can stay visibly on screen (painted, still parented)
        # for a frame or more after switching - invisible in the normal app
        # (a real event loop flushes it almost immediately) but very visible
        # doing several fast switches in a row, which tabs make routine now.
        for dw in list(self.page.widgets):
            live = self._live_by_model.get(id(dw))
            if live is not None:
                live.hide()
    ...
    def _spawn_live(self, dw: DesignWidget):
        ...
        # factory always builds at dw's raw (logical) geometry, matching
        # generated code exactly - the canvas is the one place that then
        # re-scales it for whatever the current zoom level is.
        self._apply_live_geometry(dw, live)
    ...
    def place_component(self, component_id, x=None, y=None):
        ...
        if x is None or y is None:
            ...
            # when we run out of height - never stack on one spot. Placement
            # is in logical units (like everything else in the model), so the
            # panel's actual on-screen pixel height is converted back to
            # logical before this math, or zooming in/out would change how
            # many rows fit per column.
            margin, row_h, col_w = 40, 48, 220
            per_col = max(1, (self.to_logical(self.height()) - margin * 2) // row_h)
    ...
    def commit_geometry(self, live_widget):
        ...
        if dw is None:
            return
        geo = live_widget.geometry()   # PIXEL geometry (at the current zoom)
        dw.x, dw.y = self.to_logical(geo.x()), self.to_logical(geo.y())
        dw.width, dw.height = self.to_logical(geo.width()), self.to_logical(geo.height())
    ...
    def align_selected(self, how):
        ...
        if dw is None:
            return
        # "against the canvas bounds" means the visible logical area, not the
        # raw pixel size of the panel - converting once here keeps this
        # zoom-invariant (align right lands at the same logical edge at 50%
        # or 200% zoom, exactly like it would in the generated app).
        cw, ch = self.to_logical(self.width()), self.to_logical(self.height())
        if how == "left":
            dw.x = 0
        elif how == "right":
            dw.x = max(0, cw - dw.width)
        elif how == "center":
            dw.x = max(0, (cw - dw.width) // 2)
        elif how == "top":
            dw.y = 0
        elif how == "bottom":
            dw.y = max(0, ch - dw.height)
        elif how == "middle":
            dw.y = max(0, (ch - dw.height) // 2)
        live = self._live_by_model.get(id(dw))
        if live is not None:
            self._apply_live_geometry(dw, live)
        self.sync_overlay()
    ...
    def set_geometry_of_selected(self, x, y, w, h):
        """x/y/w/h are logical units (the Properties panel edits the model
        directly), so the live widget's on-screen pixel geometry is derived
        via the current zoom, same as everywhere else."""
        dw = self.selected_model
        if dw is None:
            return
    ...
    def paintEvent(self, event):
        ...
        if self.show_grid:
            # the grid is drawn in PIXEL space, so its logical step (the
            # snap size, or the default GRID_STEP) is scaled by zoom too -
            # otherwise the grid would stay a fixed pixel size while every
            # widget on it visibly grows/shrinks, instantly looking wrong.
            step = self.snap_size if self.snap_enabled else theme.GRID_STEP
            step = max(4, int(round(step * self.zoom_factor)))
    ...
    def alignment_guides_for(self, dw):
        ...
        # canvas centre lines - dw.x/y are logical, so the panel's pixel
        # size is converted to logical too, or the centre guide would drift
        # off-centre at any zoom level other than 100%.
        targets_x = [(self.to_logical(self.width()) // 2, "centre")]
        targets_y = [(self.to_logical(self.height()) // 2, "centre")]
