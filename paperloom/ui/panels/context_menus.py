...
def build_tab_menu(parent, page, handlers, can_close_others, can_close_right) -> QMenu:
    """The menu for right-clicking an editor tab (editor_tabs.py). Rename
    edits the page's display title (what the tab itself shows) - not its
    underlying technical name, which codegen and page navigation key off of.
    Close Others/Close Tabs to the Right are disabled when there'd be nothing
    for them to do, same as a real IDE greys them out on a single tab."""
    menu = style_menu(QMenu(parent))
    menu.addAction(f"Rename “{page.title or page.name}”…", handlers["rename"])
    menu.addSeparator()
    menu.addAction("Close", handlers["close"])
    close_others = menu.addAction("Close Others", handlers["close_others"])
    close_others.setEnabled(can_close_others)
    close_right = menu.addAction("Close Tabs to the Right", handlers["close_right"])
    close_right.setEnabled(can_close_right)
    menu.addSeparator()
    menu.addAction("Save All", handlers["save_all"])
    return menu
