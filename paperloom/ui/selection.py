...
class ResizeHandle(QWidget):
    ...
    def mouseMoveEvent(self, event):
        ...
        if geo.height() < 20:
            geo.setHeight(20)
        # dragging a resize handle happens in the live widget's own (zoomed)
        # pixel space - snap_px() snaps a pixel value against the logical
        # grid (converting to logical and back), so resizing snaps to the
        # same grid the canvas draws and a move-drag snaps to, at any zoom.
        canvas = self.overlay.canvas
        geo.setLeft(canvas.snap_px(geo.left())); geo.setTop(canvas.snap_px(geo.top()))
        geo.setRight(canvas.snap_px(geo.right())); geo.setBottom(canvas.snap_px(geo.bottom()))
