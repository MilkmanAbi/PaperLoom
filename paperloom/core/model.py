...
def next_layout_id(page: "DesignPage", kind: str = "layout") -> str:
    """A LayoutGroup id unique on this page - vbox_layout, vbox_layout_2, ...
    Mirrors unique_object_name's naming convention but, unlike it, doesn't
    use a global per-process counter: a layout id only needs to be unique
    within its own page's JSON, so scanning the page directly is enough and
    avoids yet another global counter callers would need to remember to
    reset between runs (see unique_object_name / reset_ids in tests)."""
    existing = {group.id for group in page.layouts}
    n = 1
    while True:
        candidate = f"{kind}_layout" if n == 1 else f"{kind}_layout_{n}"
