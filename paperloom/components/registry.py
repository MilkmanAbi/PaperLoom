...
class Component:
    ...
    requires: list            # list[str] pip package names the PYTHON codegen
                               # path needs at RUN time (not build time) for this
                               # component to actually work - e.g. a component
                               # wrapping a third-party widget. Empty for every
                               # built-in component today; a component pack
                               # (or, later, an imported theme) opts in by adding
                               # a "requires" array to its meta.json. See
                               # core/package_retriever.py ("Package Retriever").
